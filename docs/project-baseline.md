...
当前阶段的重点不是继续堆新功能，而是在进入 Redis、Celery、PostgreSQL、RAG 异步入库等生产化增强前，建立一份清晰、可信、可维护的开发基线。
...
本阶段不引入 Redis，不引入 Celery，不切换 PostgreSQL，不做 Docker/Nginx/VPS/HTTPS 上线，不重构 RAG、Agent、LangGraph 或 Vue3 主链路。

如果在审计中发现问题，优先记录为后续阶段任务，不在本阶段扩大范围。

## 9. 下一阶段候选方向

建议下一阶段进入：

- Backend Production Infrastructure V1：保留 SQLite 本地默认开发，同时增加 PostgreSQL 配置兼容、Redis 健康检查和 Celery 异步任务基础。
- RAG 异步入库和任务化升级：将文档摄取、chunk 生成、索引构建等耗时任务迁移到任务队列。
- 管理员后台异步任务观测：展示任务状态、失败原因、重试次数和耗时。
