...
```text
Production RAG V3 已完成：RAG 文件导入任务已经从内存态升级为数据库持久化任务，补齐失败原因、重试入口、用户侧任务历史和管理员摄取任务质量监控。当前已新建 LangGraph Mainline Consolidation V7 active spec，准备把 /api/interview/next-question 默认主链路收敛到 LangGraph mainline，并把 classic Agent 降级为 fallback/helper。
```
...
第十七步：LangGraph Mainline Consolidation V7（active spec 已创建，active plan 待编写）
```
