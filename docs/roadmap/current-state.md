...
```text
暂无。Pre-Launch Delivery Roadmap V4 已完成并归档。
```
...
```text
Pre-Launch Delivery Roadmap V4 已完成：系统完成 Async Worker Readiness V4、PostgreSQL Compatibility V4、Deployment Integration V4、Project Explanation & Resume Pack V1。新增 Celery worker readiness 摘要、管理员后台异步任务 Worker 展示、Celery worker 运行说明、PostgreSQL 配置识别字段、PostgreSQL 兼容路线文档、部署配置结构化测试、稳定 Docker Compose project name、上线 runbook、pre-launch checklist、项目总讲解、面试深挖问答库、Python 后端岗简历材料、AI 应用开发岗简历材料和项目演示脚本。SQLite 仍是本地默认数据库，Redis/Celery 保持本地 eager/fallback 与生产 worker 配置兼容；真实 VPS/HTTPS 实机部署仍需用户购买服务器/域名后执行。
```

当前补充记录：

```text
Pre-Launch Delivery Roadmap V4 已归档到 `docs/specs/completed/pre-launch-delivery-roadmap-v4-design.md` 和 `docs/plans/completed/pre-launch-delivery-roadmap-v4.md`。最终验证已通过：`python -m pytest -q`、`cd frontend && npm.cmd run test`、`cd frontend && npm.cmd run build`、`docker compose config`。
```

下一阶段候选方向：

```text
当前没有 active spec / active plan。下一步建议不要继续无边界加功能，可以在真实 VPS/HTTPS 实机部署、项目讲解训练、简历投递材料精修、或少量面试体验修补之间选择。不要重复执行 Pre-Launch Delivery Roadmap V4、Backend Production Infrastructure V1、Async RAG Ingestion V2、Production Hardening V3.1 或 V3.2+V3.3。
```
...
- `tests/test_rag_ingestion_tasks.py`
- `tests/test_admin_rag_ingestion_tasks.py`
...
第十六步：Production RAG V3 摄取任务持久化与质量监控（已完成阶段性版本）
第十七步：LangGraph Mainline Consolidation V7（已完成阶段性版本）
```
...
当前 `docs/specs/active/` 应为空：

```text
暂无。
```

当前 `docs/plans/active/` 应为空：
...
LangGraph Runtime Deepening V4、LangGraph 主链路灰度迁移 V5、LangGraph / Agent 工作流深化 V6、Production RAG V3 和 LangGraph Mainline Consolidation V7 也已完成并归档。当前没有 active spec / plan，不应继续执行旧 LangGraph 或旧 RAG plan。
...
```text
已归档到 docs/specs/completed/langgraph-mainline-canary-v5-design.md
```

本阶段归档后的 active plan：

```text
已归档到 docs/plans/completed/langgraph-mainline-canary-v5.md
```
...
```text
C：面试体验与训练闭环 V3（已完成）
-> B：LangGraph / Agent 工作流深化（已完成 V6）
-> A：生产级 RAG V3（已完成）
```

## 2026-06-17 本轮补充记录：Production RAG V3 已完成

本阶段状态：

```text
Production RAG V3 已完成并归档
```
...
```text
把 RAG 文件导入任务从内存态 task_status 升级为数据库持久化任务，记录任务状态、失败原因、解析预览、生成文档、重试次数和可重试标记；用户侧能查看自己的导入任务历史，管理员后台能看到 RAG 摄取任务质量监控。
```
...
- Word / Excel / 网页完整解析。
- Qdrant / pgvector。
- 真实 Redis / Celery 队列迁移。
- Docker / Nginx / VPS 上线。

本阶段已落地：

- `RagIngestionTask` ORM 模型和 Alembic 迁移。
- RAG 上传接口写入持久化摄取任务。
- 用户侧摄取任务列表、详情和 retry 接口。
- 管理员侧 RAG 摄取任务监控接口。
- Vue3 知识库页显示最近导入任务、失败原因和重试入口。
- Vue3 管理员后台显示 RAG 摄取任务监控摘要。
- 本地 SQLite 兼容初始化补齐 `rag_ingestion_tasks` 表。

本阶段已验证：

```text
backend focused:
python -m pytest tests/test_rag_ingestion_tasks.py tests/test_rag_documents_upload_route.py tests/test_admin_rag_ingestion_tasks.py tests/test_admin_rag_quality.py -q
结果：11 passed, 1 warning

backend full:
python -m pytest -q
结果：363 passed, 1 warning

frontend full:
cd frontend
npm.cmd run test
结果：30 个测试文件通过，119 个测试通过
...
browser:
- /vue/app/knowledge 桌面端和移动端：显示最近导入任务区域，无 undefined，无横向溢出。
- /vue/app/admin 桌面端和移动端：显示 RAG 摄取任务监控、失败任务和可重试指标，无 undefined，无横向溢出。
```

## 2026-06-17 本轮补充记录：LangGraph Mainline Consolidation V7 已完成

本阶段状态：

```text
LangGraph Mainline Consolidation V7 已完成并归档。
```
...
```text
将 /api/interview/next-question 默认内部运行时从 classic Python Orchestrator 收敛到 LangGraph mainline，classic Agent 降级为 fallback/helper；同时保持前端请求路径兼容，并让 Vue3 面试页和管理员后台承接工作流解释与可观测性信息。
```

本阶段已落地：

- `runtime_policy.py` 默认策略切换为 `langgraph_mainline`，非法 runtime 也归一到 mainline，显式 `classic` 仍保留为 fallback/debug runtime。
- `agent_runtime.py` 新增 `langgraph_mainline` 分支，LangGraph 质量门禁通过时直接作为可见主链路，失败时回退 classic 并写入 runtime audit。
- `/api/interview/next-question` 默认进入 runtime layer，响应兼容原字段，并新增可选 `workflowTrace`、`checkpointSummary`、`qualityGate`、`fallbackSummary`。
- RAG 没有重写检索算法，只通过 LangGraph `retrieve_context` 节点复用现有三类 RAG 能力。
- AI Debug detail 新增 `workflowObservation`，聚合 runtime、fallback、checkpoint、nodeTrace 和 RAG 摘要。
- Vue3 面试页保存 runtime/workflow 摘要，并展示轻量的“为什么这么问”和稳定兜底提示。
- Vue3 管理员后台新增“Agent 工作流观测”主诊断区，展示 runtime、稳定兜底、quality gate、checkpoint、节点轨迹和 RAG 节点摘要。

本阶段明确未做：

- 未重构 BM25、hybrid search、rerank、RAG 文档管理和摄取任务。
- 未新增 OCR、Word、Excel、网页解析。
- 未接入 Qdrant / pgvector。
...
browser:
- /vue/app/interview 桌面端：可进入面试页，当前无档案时显示创建档案引导，无 undefined，无横向溢出。
